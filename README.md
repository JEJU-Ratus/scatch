...
- 로그인 아이디는 소문자 알파벳(a-z)또는 0~9로 시작하며 소문자 알파벳(a-z),숫자,-,_ 를 이용해 5자 이상 21자 이내임.

- 로그인 패스워드는 최소, 하나의 숫자(0~9)와 하나의 a-z와 하나의 특수문자(!,@,#) 가 들어가야함.
- 가능한 문자는 숫자, 소문자 알파벳(a-z), 대문자 알파벳(A-Z), 그리고 특수 문자(!, @, #)입니다.
- 전체 길이는 9~20자 사이여야 합니다.
